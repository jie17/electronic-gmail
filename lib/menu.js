...
const darwinTpl = [{
    ...
    submenu: [{
        ...
        label: 'Zoom to 150%',
        click(item, focusedWindow) {
            focusedWindow.webContents.send('zoom', 1.5);
            config.setZoomFactor(1.5);
        }
    }, {
        label: 'Zoom to 125%',
        click(item, focusedWindow) {
            focusedWindow.webContents.send('zoom', 1.25);
            config.setZoomFactor(1.25);
        }
    }, {
        label: 'Zoom to 100%',
        click(item, focusedWindow) {
            focusedWindow.webContents.send('zoom', 1);
            config.setZoomFactor(1);
        }
    }]
}];

module.exports = electron.Menu.buildFromTemplate(darwinTpl);